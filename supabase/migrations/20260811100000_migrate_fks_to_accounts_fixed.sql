...
-- Re-add author_student_id constraint to accounts.ctu_id
-- (only if the column exists and has data)
DO $$ 
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.columns 
             WHERE table_name = 'announcements' AND column_name = 'author_student_id') THEN
    ALTER TABLE announcements 
...
ALTER TABLE message_reactions 
  DROP CONSTRAINT IF EXISTS message_reactions_student_id_fkey;

-- Note: message_reactions.student_id is text, should reference accounts.ctu_id
ALTER TABLE message_reactions 
  ADD CONSTRAINT message_reactions_student_id_fkey 
...
-- Note: messages.student_id is text, should reference accounts.ctu_id
ALTER TABLE messages 
...
-- =========================================================
-- VERIFICATION: Run this to see all constraints now point to accounts
-- =========================================================
-- SELECT 
