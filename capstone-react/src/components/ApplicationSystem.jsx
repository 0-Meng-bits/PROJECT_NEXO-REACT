...
export function ApplicationStatusLabel(status, phase2Result) {
...
export function ApplicationStatusColor(status, phase2Result) {
...
// -- Application FORM BUILDER (leader side) --------------------------------------
// Now supports multiple named Applications per circle
export function ApplicationFormBuilder({ comm, onToggle }) {
  const [Applications, setApplications] = useState([]);
  const [loading, setLoading] = useState(true);
  const [creating, setCreating] = useState(false);
  const [selectedApplication, setSelectedApplication] = useState(null);
  const [newForm, setNewForm] = useState({ title: '', description: '', type: 'external', post_to_feed: false });
  const [newQuestions, setNewQuestions] = useState([]); // questions for the new Application being created
  ...
  useEffect(() => {
    if (selectedApplication) loadQuestions(selectedApplication.id);
  }, [selectedApplication, loadQuestions]);
    ...
    if (!error && data) {
      ...
      if (newForm.type === 'external') {
        ...
        await supabase.from('announcements').insert([{
          ...
          author_type: 'Leader',
          title: 'Application Open - ' + data.title,
        ...
        if (newForm.post_to_feed) {
          await supabase.from('announcements').insert([{
            ...
            author_name: comm.name,
            author_type: 'Leader',
            title: 'Application Open - ' + data.title + ' (' + comm.name + ')',
        ...
        await supabase.from('announcements').insert([{
          ...
          author_type: 'Leader',
          title: 'Internal Application Open - ' + data.title,
          content: (newForm.description || 'An internal Application is now open for current members. Click Apply Now to participate.'),
          post_type: 'announcement',
          community_id: comm.id,
        }]);
      }
      // Save questions for this Application
      ...
      // Also update community application_enabled flag
      await supabase.from('communities').update({
        application_enabled: true,
        internal_application: newForm.type === 'internal',
      }).eq('id', comm.id);
      onToggle(true);
  ...
  if (loading) return <p style={{ color: 'var(--text-muted)', fontSize: 13 }}>Loading Applications...</p>;
    ...
    return (
      <div className="Application-builder">
        <button onClick={() => setSelectedApplication(null)}
          ...
          <i className="fa-solid fa-arrow-left"></i> Back to all Applications
        </button>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 16 }}>
          <div>
            <div style={{ fontWeight: 700, fontSize: 15, color: 'var(--text-primary)' }}>{selectedApplication.title}</div>
            <div style={{ fontSize: 11, color: selectedApplication.type === 'internal' ? 'var(--cyber-yellow)' : 'var(--cyber-cyan)', marginTop: 4 }}>
              <i className={selectedApplication.type === 'internal' ? 'fa-solid fa-lock' : 'fa-solid fa-globe'} style={{ marginRight: 5 }}></i>
              {selectedApplication.type === 'internal' ? 'Internal (Members Only)' : 'External (Open to All)'}
              {selectedApplication.type === 'external' && selectedApplication.post_to_feed && (
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            <button className="adm-btn" onClick={() => toggleApplicationOpen(selectedApplication)}
              style={{ color: selectedApplication.is_open ? 'var(--green)' : 'var(--text-muted)', borderColor: selectedApplication.is_open ? 'var(--green)' : '#333' }}>
              <i className={selectedApplication.is_open ? 'fa-solid fa-door-open' : 'fa-solid fa-door-closed'}></i>
              {selectedApplication.is_open ? 'Open' : 'Closed'}
            </button>
            <button className="adm-btn reject" onClick={() => deleteApplication(selectedApplication.id)}>
              <i className="fa-solid fa-trash"></i>
            </button>
          </div>
        </div>

        <div className="Application-section-label">
          <span>Questions</span>
    ...
    <div className="Application-builder">
      ...
      <div style={{ background: 'rgba(0,240,255,0.04)', border: '1px solid rgba(0,240,255,0.15)', borderRadius: 10, padding: 16, marginBottom: 20 }}>
        <div style={{ fontWeight: 700, fontSize: 13, color: 'var(--cyber-cyan)', marginBottom: 12 }}>
          <i className="fa-solid fa-plus" style={{ marginRight: 8 }}></i>Create New Application
        </div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 12, padding: '10px 12px', background: 'rgba(62,207,142,0.05)', border: '1px solid rgba(62,207,142,0.2)', borderRadius: 8 }}>
            ...
            <div>
              ...
              <div style={{ fontSize: 10, color: 'var(--text-muted)', marginTop: 2 }}>
                Announce this Application to all students on the home feed
              </div>
            </div>
          </div>
        )}
        ...
        <button className="cyber-btn" onClick={createApplication} disabled={creating || !newForm.title.trim()} style={{ width: '100%', marginTop: 14 }}>
          {creating ? 'Creating...' : <><i className="fa-solid fa-microphone" style={{ marginRight: 6 }}></i>Create Application</>}
        </button>
      </div>
      ...
      {Applications.length === 0 ? (
        <p style={{ color: 'var(--text-muted)', fontSize: 12, padding: '12px 0' }}>No Applications yet. Create one above.</p>
      ) : (
        Applications.map(aud => {
          const count = aud.application_submissions?.[0]?.count || 0;
          return (
            <div key={aud.id} className="Application-response-row" onClick={() => setSelectedApplication(aud)}
...
export function ApplicationApplicationForm({ comm, Application, applicantId, onSubmitted, onCancel }) {
  ...
  const handleFileUpload = async (questionId, file) => {
    ...
    const path = 'Applications/' + comm.id + '/' + applicantId + '/' + questionId + '.' + ext;
    const { error } = await supabase.storage.from('Application-files').upload(path, file, { upsert: true });
    if (!error) {
    ...
    <div className="modal-overlay" onClick={onCancel}>
      <div className="modal-box" style={{ maxWidth: 520, maxHeight: '85vh', overflowY: 'auto' }} onClick={e => e.stopPropagation()}>
        <h3><i className="fa-solid fa-microphone" style={{ marginRight: 8 }}></i>
          {Application ? Application.title : 'Apply to ' + comm.name}
        </h3>
        <p style={{ color: 'var(--text-muted)', fontSize: 12, marginBottom: 20, marginTop: -10 }}>
          {Application?.description || 'Fill out the form below. The circle leader will review your application.'}
        </p>
        {questions.length === 0 && (
...
// -- Application REVIEW PANEL (leader side) --------------------------------------
export function ApplicationReviewPanel({ comm }) {
  ...
  const act = async (responseId, status, extra = {}) => {
    ...
    const { error } = await supabase.from('application_submissions').update(update).eq('id', responseId);
    if (!error) {
      ...
      if (resp?.applicant_id) {
        let message = '';
        const audTitle = resp.Applications?.title || comm.name;
        if (status === 'phase2') message = 'Your Application for "' + audTitle + '" passed Phase 1! Check your application for live screening details.';
        else if (status === 'accepted') message = 'Congratulations! Your Application for "' + audTitle + '" has been accepted.';
        else if (status === 'rejected') message = 'Your Application application for "' + audTitle + '" was not accepted this time.';
        ...
        if (message) {
          await supabase.from('notifications').insert([{
            user_id: resp.applicant_id, type: 'Application_update', message, link_comm_id: comm.id,
          }]);
        }
      }
      await load();
  ...
  if (responses.length === 0) return <p style={{ color: 'var(--text-muted)', fontSize: 13, padding: '20px 0' }}>No Application applications yet.</p>;

  return (
    <div>
      {!selected && responses.map(r => (
        <div key={r.id} className="Application-response-row" onClick={() => setSelected(r)}>
          <div className="member-card-avatar" style={{ width: 38, height: 38, fontSize: 13 }}>
            {r.accounts?.full_name?.[0] || '?'}
          </div>
          <div style={{ flex: 1 }}>
            <div style={{ fontWeight: 600, fontSize: 13, color: 'var(--text-primary)' }}>{r.accounts?.full_name}</div>
            <div style={{ fontSize: 11, color: 'var(--text-muted)' }}>
              {r.accounts?.ctu_id} � {r.Applications?.title || 'General'} � {new Date(r.submitted_at).toLocaleDateString()}
            </div>
          </div>
          <span style={{ fontSize: 10, fontWeight: 700, padding: '3px 10px', borderRadius: 20,
            color: ApplicationStatusColor(r.status, r.phase2_result),
            border: '1px solid ' + ApplicationStatusColor(r.status, r.phase2_result),
            background: ApplicationStatusColor(r.status, r.phase2_result) + '15' }}>
            {ApplicationStatusLabel(r.status, r.phase2_result)}
          </span>
          <i className="fa-solid fa-chevron-right" style={{ color: 'var(--text-muted)', fontSize: 11, marginLeft: 8 }}></i>
        </div>
      ))}
      ...
      {selected && (
        <div>
          ...
          <div style={{ marginBottom: 20 }}>
            <div style={{ fontWeight: 700, fontSize: 16, color: 'var(--text-primary)' }}>{selected.accounts?.full_name}</div>
            ...
            <div style={{ marginTop: 20 }}>
              ...
              {selected.status === 'pending' && (
                <div className="input-group">
                  ...
                  <textarea style={{ width: '100%', background: '#000', border: '1px solid #333', padding: 10, color: 'white', borderRadius: 6, fontFamily: 'inherit', fontSize: 12, outline: 'none', height: 70, resize: 'none' }}
                    placeholder="e.g. Live Application on June 5 at 3PM - Room 201"
